...
public:
	...
	VulkanResult<VulkanAllocation>
		CreateBuffer(vk::DeviceSize size, vk::BufferUsageFlags usage, const VulkanAllocationCreateInfo& allocInfo);

	void DestroyBuffer(vk::Buffer buffer, VmaAllocationHandle allocation);

	// Image allocation
	VulkanResult<VulkanAllocation>
		CreateImage(const vk::ImageCreateInfo& imageInfo, const VulkanAllocationCreateInfo& allocInfo);
	...
	VmaAllocatorHandle Handle() const noexcept
	{
		return allocator_;
	}
